...
chromosome_heatmap <- read_tsv(file = "Data/Virulence/Chromosome/all_chromosome.tsv",
...
###Plasmids
```{r}
plasmids_heatmap <- read_tsv(file = "Data/Virulence/Plasmids/all_plasmids.tsv",
...
ggsave("Figure2.pdf", plot = Figure2, device = cairo_pdf, path = "Plots/", height = 30, width = 40, units = "cm", limitsize = FALSE)
ggsave("Figure2.png", plot = Figure2, device = "png", path = "Plots/", height = 30, width = 40, units = "cm", limitsize = FALSE)
...
plotGenTime <- function(DNADist, GeoDist, TimeDist, mantelResults) {
  ...
  GenTimeCorr <- ggplot(GenGeoTimeD, aes(x= GenD, y=C14_distance, color=GeoD)) +
    ...
    labs(title = paste("Mantel Statistic r =", round(mantelResults$statistic, digits = 4), ", Significance =", round(mantelResults$signif, digits = 4)), x = "Genetic distance", y= "Temporal distance", colour= "Geographical \n distance") +
    theme_bw()
  return(GenTimeCorr)
}
...
#    geom_smooth() +
    scale_color_viridis_c(option = "plasma") +
    labs(title = paste("Mantel Statistic r =", round(mantelResults$statistic, digits = 4), ", Significance =", round(mantelResults$signif, digits = 4)), x = "Genetic distance", y= "Geographical distance", colour= "Temporal \n distance") +
    theme_bw()
  return(GenGeoCorr)
}
...
snpTable <- read.delim("Data/2020-07-09_LNBA_leprosy_enterica_comp/LNBA_transect/LNBAdataset2021_SNPexcluded_genotyped.tsv", check.names = FALSE) %>%
...
##Reading snpTable and selecting genomes present in the metadata file
snpTable <- read.delim("Data/2020-07-09_LNBA_leprosy_enterica_comp/LNBA_transect/LNBAdataset2021_SNPexcluded_genotyped.tsv") %>%
  pivot_longer(c(-Position, -Ref), names_to = "Genomes", values_to = "Call") %>%
  filter(Genomes %in% metadata$Name) %>%
  mutate(CallDef = ifelse(Call == ".", as.character(Ref), as.character(Call))) %>%
  select(-Ref, -Call)
...
metadataLNBA$Sample.Name <- factor(metadataLNBA$Sample.Name, levels= c("RK1001", "RISE509", "VLI092","KNK001","GRS004", "GEN72","Gyvakaral1","VEL003","I5884","GLZ001","GLZ002","KunilaII","1343UNTAL85","HOP001", "HOP004","KLZ001","CHC004","6POST","KLE031", "KLE048","MIB054","RISE505","OOH003","ARS007","GRH001", "KZL002","I2470","RT5"))
...
radiocarbon_plot <- ggplot() +
  ...
  scale_color_manual(values = c("#00bfc4ff","#f8766dff")) +
...
metadataLNBA$Published <- factor(metadataLNBA$Published, levels = c("This study","Yu et al. (2020)","Spyrou et al. (2018)","Andrades Valtueña et al. (2017)", "Rasmussen et al. (2015)"))
...
Map <- ggmap(map, aes(label =metadataLNBA$Site)) + 
 ...
 xlab("Latitude") + 
 ylab("Longitude") +
  scale_fill_manual(name = "Clade", values = c("#9600bf99","#22ad80ff")) +
  scale_shape_manual(name = "Publication",values = c(24,25,23,21,22)) +
  scale_x_continuous(expand=c(0,0))+
  scale_y_continuous(expand=c(0,0)) +
  guides(fill = guide_legend(override.aes = list(shape =21)),
         shape = guide_legend(override.aes = list(fill = "black", size = 3))) +
  theme_bw() +
  theme(legend.position = c(0.88,0.157),
